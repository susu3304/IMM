...
marmot main {
  ...
  },
  {
    ...
    coverage: "partial",
    coverageNote: "squeak/trace/sniff は WASM runner と native/API runtime で利用可能",
    ...
    bullets: [
      ...
      "`sniff` は stdin から読むための式で、Web Runner の stdin textarea や API の `stdin` フィールドから渡された入力を読みます。",
      "local API runner は stdin を一時ファイルへ書き、`IMM_STDIN_FILE` を runtime に渡します。",
...
marmot main {
  ...
  },
  {
    ...
    coverage: "partial",
    coverageNote: "native async done / browser uses JS host timers",
    ...
    bullets: [
      ...
      "`nap(ms)` は native runtime の non-blocking sleep で、Browser WASM では JS host timer 経由で待機します。",
...
howl marmot main {
  ...
  },
  {
    ...
    coverage: "partial",
    coverageNote: "core/math は WASM runner でも利用可能。nap と tick.now は native/API runtime と Browser WASM JS host",
    ...
    bullets: [
      ...
      "`tick.now()` は native/API runtime と Browser WASM JS host で UNIX milliseconds を返します。",
      "`nap(ms)` は native howl task または Browser WASM JS host の timer として sleep します。",
...
marmot main {
  ...
  },
  {
    ...
    coverage: "partial",
    coverageNote: "Browser WASM は JS fetch host、native/API は native HTTP client",
    ...
    bullets: [
      ...
      "Browser WASM runtime では JS fetch host 経由で `web.grab` / `web.fetch` を実行し、HTTP(S) はブラウザの CORS 制約に従います。",
      "native/API runtime は native HTTP client で実行し、閉じた環境では runtime API の認可境界で制御します。",
...
imm pack main.imm --crate dist/main-native --pelt native`
  },
  {
    ...
    coverage: "partial",
    coverageNote: "Browser WASM and runtime API share imm-core with different host capabilities",
    ...
    bullets: [
      ...
      "WASM runner は worker と 3 秒タイムアウトで隔離され、stdin、JS fetch host、`tick.now()`、`nap(ms)` を Browser WASM の host capability として受け取ります。",
      "Browser WASM の `web.grab` / `web.fetch` はブラウザの fetch と CORS に従い、OS sandbox、ファイル永続化、native TCP server、native HTTP client は持ちません。",
      "native CLI/API runtime は `native` feature を使い、HTTP client/server、store、pack、law suite を含みます。",
      "local API runner は 64KB のソース上限、64KB の stdin 上限、3 秒標準タイムアウト、最大 8 秒タイムアウト、stdout/stderr 各 64KB 上限を持ちます。",
      "公開 imm-web は Browser WASM 実行を標準にし、閉じた環境では API key 付き runtime API を接続する構成にできます。",
